...
describe("GET /users/:username", function () {
  ...
  test("works for ident user", async function(){
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
  ...
  test("unauth with other users", async function(){
    const resp = await request(app)
        .get(`/users/u1`)
...
describe("PATCH /users/:username", () => {
  ...
  test("unauth if not ident user", async function(){
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
...
describe("POST /users/:username/jobs/:id", function(){
  ...
  test("no such username found", async function(){
    const resp = await request(app)
